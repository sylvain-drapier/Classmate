# from tests_manu import emp_pan
# from tests_manu import T300_914_emp0_bis
from utils import np, tk, ttk, Image, ImageTk

import Contraintes_plaques as cont
from CalculResistances import CalculResistances
import interface as interface



class Int_contr_plaque:
    def __init__(self, Empilement):
        ...
        # Six Entry Boxes
        self.entry_widgets = []
        noms_eff_gen = ['N11', 'N22', 'N12', 'M11', 'M22', 'M12', 'Q2'] 
        for i in range(0, len(noms_eff_gen)):
        ...
        x, y = self.position_pr_aff("moy_champs", 51)
        x1, y1 = self.position_pr_aff("moy_champs", 52)
        nom_coef = tk.Label(root, text='Q1')
        nom_coef.place(x = x, y = y)
        val_ef_gen = tk.Entry(self.root)
        val_ef_gen.insert(0, "0")
        self.entry_widgets.append(val_ef_gen)
    ...
    def update_values(self):
        ...
        coord = [0, 0, float(self.z_entree.get())]
        val = []
        val_pli = []
        val_max = []
            
        self.calculateur_cont = cont.CalculContraintesPlaques(self.empilement)
        
        # On calcul les propriétés
        tableau_coord, hauteur_grid, largeur_grid = self.calculateur_cont.tableau_coord(self.nb_pt_pr_aff)
        contraintes_brin, def_brin = self.calculateur_cont.contraintes_et_def_section(eff_gen, tableau_coord, critere_rupture, 'total')
        ...
        for mode in ['def', 'cont']:
            ...
            for ind_voigt in [1,2,4,5,6]:
                cont.CalculContraintesPlaques.dessin_section(self.calculateur_cont, tableau, largeur_grid, hauteur_grid, ind_voigt, mode)
        
        # On affiche les dessins
        img = ['sig_1', 'sig_2', 'sig_4', 'sig_5', 'sig_6', 'def_1', 'def_2', 'def_4', 'def_5', 'def_6']
        ...
        tabl_max = np.zeros((10, 6))
        row_index = 0
        for prefix in ['cont', 'def']:
            for indice_voigt in [1,2,4,5,6]:
    ...
    def tableaux(self, type_table, leg, val):
        if type_table == 'val_ponct_struct' or type_table == 'val_ponct_pli' :
            ...
            nbr_cine = 5
            data = [[0]*4 for _ in range(nbr_cine)]
            for k in range(len(leg)//2):
                data[k % nbr_cine][0] = leg[k]
                if val[k] != 0:
                    data[k % nbr_cine][1] = f"{val[k]:.1e}" # round(val[k], 2) 
                else:
                    data[k % nbr_cine][1] = "0"
                data[k % nbr_cine][2] = leg[k + nbr_cine]
                if val[k+nbr_cine] != 0:
                    data[k % nbr_cine][3] = f"{val[k+nbr_cine]:.1e}" # round(val[k], 2) 
                else:
                    data[k % nbr_cine][3] = "0"
            for row in data:
            ...
            self.tree.place(x=x, y=y, width=self.table_width, height=1.4*self.table_height)
            ...
            for i in range(nb_l):
                ...
                #     data[i][3] = "0"
                if val[i,offset + 2] != 0:
                    data[i][2] = f"{val[i,offset + 2]:.1e}" # round(val[k], 2) 
                else:
                    data[i][2] = "0"
            for row in data:
                self.tree.insert('', 'end', values=row)
            ...
            x, y = self.position_pr_aff("moy_champs", pos_tableau)
            self.tree.place(x=x, y=y, width=self.table_width, height=2.5 * self.table_height)
            ...
            z_enca.place(x = x, y = y)
            #columns = ('Critère', 'Réserve', 'x2', 'x3')
            columns = ('Critère', 'Réserve', 'x3', 'ind_pli')
            self.tree = ttk.Treeview(self.root, columns=columns, show='headings')
    ...
    def position_pr_aff(self, taille, pos):
        ...
        if pos == -1 or pos == 1 or pos == 3 or pos == 5 or pos == 7 or pos == 9 or pos == 11 or pos == 44 or pos == 13 or pos == 51:
            x = 0
        elif pos == 0 or pos == 2 or pos == 4 or pos == 6 or pos == 8 or pos == 10 or pos == 12 or pos == 14 or pos == 52:
        ...
        elif pos == 13 or pos == 14:
            y = hauteur * 7 #incr_part_hg * 6 + hauteur * 5
        elif pos == 51 or pos == 52:
            y = hauteur * 8 #incr_part_hg * 6 + hauteur * 5
        elif pos == 44:
        ...
        incr_part_hd_larg = (width*5/4 - 5*ta)/6
        
        if pos == 20 or pos == 21 or pos == 22  or pos == 23  or pos == 24:
            y = incr_part_hd_haut
        elif pos == 25 or pos == 53 or pos == 54  or pos == 55 or pos == 56:
            y = incr_part_hd_haut * 2 + hauteur
            
        offset_larg = 20
        if pos == 20 or pos == 25:
            x = width/4
        elif pos == 21 or pos ==  53:
            x = width/4 + largeur + offset_larg
        elif pos == 22 or pos == 54:
            x = width/4 + 2 * largeur + 2 * offset_larg
        elif pos == 23 or pos == 55:
            x = width/4 + 3 * largeur + 3 * offset_larg
        elif pos == 24 or pos == 56:
            x = width/4 + 4 * largeur + 4 * offset_larg
        ...
        if pos == 30 or pos == 35 or pos == 39:
            ...
            y = height/2 + height/50# height/2 + 2 * incr_part_milieu_bas_haut + self.champs_height - offset
            ...
            y = height*3/4 + height/50 - height/12#height/2 + 4 * incr_part_milieu_bas_haut + 2 * self.champs_height + self.updated_height - offset
        elif pos == 34:
    ...
    def Set_image(self, figure):
        ...
        figure_mapping = {
            'sig_1': (20, 'Ressources/images_gen/figure_plaque_section_contraintes_1.png'),
            'sig_2': (21, 'Ressources/images_gen/figure_plaque_section_contraintes_2.png'),
            'sig_4': (22, 'Ressources/images_gen/figure_plaque_section_contraintes_4.png'),
            'sig_5': (23, 'Ressources/images_gen/figure_plaque_section_contraintes_5.png'),
            'sig_6': (24, 'Ressources/images_gen/figure_plaque_section_contraintes_6.png'),
            'def_1': (25, 'Ressources/images_gen/figure_plaque_section_def1.png'),
            'def_2': (53, 'Ressources/images_gen/figure_plaque_section_def2.png'),
            'def_4': (54, 'Ressources/images_gen/figure_plaque_section_def4.png'),
            'def_5': (55, 'Ressources/images_gen/figure_plaque_section_def5.png'),
            'def_6': (56, 'Ressources/images_gen/figure_plaque_section_def6.png'),
            'rupt': (41, 'Ressources/images_gen/figure_section_rupture_plaque.png'),
            }
        
        for figure, (label_number, image_path) in figure_mapping.items():
            #P Thomas, ajout de la condition pour étirer l'image rupt
            if figure != 'rupt':
    ...
    def Set_image_init(self, figure):
        ...
        if figure == 'dess_emp':
            ...
            image_1 = Image.open("Ressources/images_fixes/plaque_efforts.png")
...
if __name__ == "__main__":
    # app = Int_contr_plaque() # T300_914_emp0_bis, emp_pan
    app = Int_contr_plaque(emp_pan) # T300_914_emp0_bis, emp_pan
